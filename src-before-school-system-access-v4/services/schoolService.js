...
// ============================================================
// SCHOOL S17 — Smart Timetable
// ============================================================
export async function getSchoolS17Health(){
 const {data,error}=await supabase.rpc("school_s17_health");
 if(error)throw error;return data||{};
}
export async function getSchoolPeriodSettings(){
 const {data,error}=await supabase.from("school_period_settings").select("*").eq("is_active",true).order("period_no");
 if(error)throw error;return data||[];
}
export async function saveSchoolPeriodSetting(x){
 const row={
  academic_year_id:x.academic_year_id,
  period_no:Number(x.period_no),
  starts_at:x.starts_at,
  ends_at:x.ends_at,
  label_ar:x.label_ar||null,
  label_en:x.label_en||null,
  is_break:!!x.is_break,
  is_active:x.is_active!==false,
  updated_at:new Date().toISOString()
 };
 if(x.id){const{data,error}=await supabase.from("school_period_settings").update(row).eq("id",x.id).select().single();if(error)throw error;return data}
 const{data,error}=await supabase.from("school_period_settings").upsert(row,{onConflict:"academic_year_id,period_no"}).select().single();if(error)throw error;return data;
}
export async function getSchoolTimetableWeeklyLoad(sectionId){
 const {data,error}=await supabase.rpc("school_timetable_weekly_load",{p_class_section_id:sectionId});
 if(error)throw error;return data||[];
}
export async function saveSchoolTimetableEntrySmart(x){
 const {data,error}=await supabase.rpc("school_save_timetable_entry",{
  p_id:x.id||null,
  p_academic_year_id:x.academic_year_id,
  p_class_section_id:x.class_section_id,
  p_subject_id:x.subject_id,
  p_teacher_id:x.teacher_id,
  p_weekday:Number(x.weekday),
  p_period_no:Number(x.period_no),
  p_room:x.room||null
 });
 if(error)throw error;return data;
}

export async function getSchoolS18Health(){const {data,error}=await supabase.rpc("school_s18_health");if(error)throw error;return data||{}}
export async function getSchoolMonthlyAttendanceReport(sectionId,year,month){const {data,error}=await supabase.rpc("school_attendance_monthly_report",{p_class_section_id:sectionId,p_year:Number(year),p_month:Number(month)});if(error)throw error;return data||[]}
export async function getSchoolStudentAffairsProfile(studentId){const {data,error}=await supabase.rpc("school_student_affairs_profile",{p_student_id:studentId});if(error)throw error;return data||{}}
export async function addSchoolStudentAffairsNote(x){const {data,error}=await supabase.rpc("school_add_student_affairs_note",{p_student_id:x.student_id,p_enrollment_id:x.enrollment_id||null,p_note_type:x.note_type||"general",p_severity:x.severity||"normal",p_title:x.title,p_note:x.note,p_action_taken:x.action_taken||null,p_follow_up_date:x.follow_up_date||null,p_is_private:x.is_private!==false});if(error)throw error;return data}
export async function reviewSchoolAttendanceExcuse(id,status,note=""){const {data,error}=await supabase.rpc("school_review_attendance_excuse",{p_excuse_id:id,p_status:status,p_review_note:note||null});if(error)throw error;return data}
export async function getSchoolAttendanceExcuses(){const {data,error}=await supabase.from("school_attendance_excuses").select("*,school_enrollments(id,school_students(id,student_no,full_name_ar))").order("created_at",{ascending:false});if(error)throw error;return data||[]}


// ============================================================
// SCHOOL S19 — Unified Gradebook
// ============================================================
export async function getSchoolS19Health(){
 const {data,error}=await supabase.rpc("school_s19_health");
 if(error)throw error;return data||{};
}
export async function getSchoolGradebookClass(sectionId,subjectId,termNo){
 const {data,error}=await supabase.rpc("school_gradebook_class",{
  p_class_section_id:sectionId,p_subject_id:subjectId,p_term_no:Number(termNo)
 });
 if(error)throw error;return data||{};
}
export async function rebuildSchoolSubjectGradebook(sectionId,subjectId,termNo){
 const {data,error}=await supabase.rpc("school_rebuild_subject_gradebook",{
  p_class_section_id:sectionId,p_subject_id:subjectId,p_term_no:Number(termNo)
 });
 if(error)throw error;return data;
}
export async function saveSchoolGradebookSetting(x){
 const {data,error}=await supabase.rpc("school_save_gradebook_setting",{
  p_academic_year_id:x.academic_year_id,
  p_grade_level_id:x.grade_level_id,
  p_curriculum_id:x.curriculum_id,
  p_subject_id:x.subject_id,
  p_term_no:Number(x.term_no),
  p_assignments_weight:Number(x.assignments_weight),
  p_exams_weight:Number(x.exams_weight),
  p_pass_mark:Number(x.pass_mark)
 });
 if(error)throw error;return data;
}
export async function publishSchoolTermGradebook(sectionId,termNo){
 const {data,error}=await supabase.rpc("school_publish_term_gradebook",{
  p_class_section_id:sectionId,p_term_no:Number(termNo)
 });
 if(error)throw error;return data||{};
}
export async function rebuildSchoolAnnualGradebook(sectionId){
 const {data,error}=await supabase.rpc("school_rebuild_annual_gradebook",{p_class_section_id:sectionId});
 if(error)throw error;return data;
}


// ============================================================
// SCHOOL S20.1 / S20.2 — Finance Complete
// ============================================================
export async function getSchoolFinanceDashboard(){
 const {data,error}=await supabase.rpc("school_finance_dashboard");
 if(error)throw error;return data||{};
}
export async function getSchoolEnrollmentFinanceSummary(enrollmentId){
 const {data,error}=await supabase.rpc("school_enrollment_finance_summary",{p_enrollment_id:enrollmentId});
 if(error)throw error;return data||{};
}
export async function applySchoolSiblingDiscount(enrollmentId){
 const {data,error}=await supabase.rpc("school_apply_sibling_discount",{p_enrollment_id:enrollmentId});
 if(error)throw error;return data||{};
}
export async function createSchoolFeeReceipt(x){
 const {data,error}=await supabase.rpc("school_create_fee_receipt",{
  p_enrollment_id:x.enrollment_id,
  p_installment_id:x.installment_id||null,
  p_amount:Number(x.amount),
  p_currency:x.currency||"SAR",
  p_payment_method:x.payment_method||"cash",
  p_reference_no:x.reference_no||null,
  p_notes:x.notes||null
 });
 if(error)throw error;return data||{};
}
export async function getSchoolFeeReceipts(){
 const {data,error}=await supabase.from("school_fee_receipts")
  .select("*,school_enrollments(id,school_students(id,student_no,full_name_ar))")
  .order("paid_at",{ascending:false});
 if(error)throw error;return data||[];
}
export async function getSchoolFeeAdjustments(){
 const {data,error}=await supabase.from("school_fee_adjustments")
  .select("*,school_enrollments(id,school_students(id,student_no,full_name_ar))")
  .order("created_at",{ascending:false});
 if(error)throw error;return data||[];
}
export async function getSchoolS20Health(){
 const {data,error}=await supabase.rpc("school_s20_health");
 if(error)throw error;return data||{};
}


// ============================================================
// SCHOOL S21 — HR & Staff Management
// ============================================================
export async function getSchoolS21Health(){const{data,error}=await supabase.rpc("school_s21_health");if(error)throw error;return data||{}}
export async function getSchoolHRDashboard(){const{data,error}=await supabase.rpc("school_hr_dashboard");if(error)throw error;return data||{}}
export async function getSchoolEmployeeHRProfile(id){const{data,error}=await supabase.rpc("school_employee_hr_profile",{p_employee_id:id});if(error)throw error;return data||{}}
export async function saveSchoolPerformanceReview(x){const{data,error}=await supabase.rpc("school_save_performance_review",{p_id:x.id||null,p_employee_id:x.employee_id,p_start:x.review_period_start,p_end:x.review_period_end,p_attendance_score:Number(x.attendance_score||0),p_performance_score:Number(x.performance_score||0),p_behavior_score:Number(x.behavior_score||0),p_strengths:x.strengths||null,p_improvement_areas:x.improvement_areas||null,p_manager_comment:x.manager_comment||null,p_final:!!x.final});if(error)throw error;return data}


// ============================================================
// SCHOOL S22 FINAL — Production & Control Center
// ============================================================
export async function getSchoolS22Health(){
 const {data,error}=await supabase.rpc("school_s22_health");
 if(error)throw error;return data||{};
}
export async function getSchoolControlCenter(){
 const {data,error}=await supabase.rpc("school_control_center");
 if(error)throw error;return data||{};
}
export async function getSchoolFinalReadiness(){
 const {data,error}=await supabase.rpc("school_final_readiness");
 if(error)throw error;return data||{};
}
export async function getSchoolExportSummary(){
 const {data,error}=await supabase.rpc("school_export_summary");
 if(error)throw error;return data||{};
}
