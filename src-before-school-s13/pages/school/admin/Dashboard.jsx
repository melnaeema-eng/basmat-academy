...
import {getSchoolCore,getSchoolS2Health,getSchoolS3Health,getSchoolS4Health,getSchoolS5Health,getSchoolS6Health,getSchoolS7Health,getSchoolS8Health,getSchoolS9Health,getSchoolS10Health,getSchoolS11Health,getSchoolS12Health} from "../../../services/schoolService";

export default function SchoolDashboard(){
 const[data,setData]=useState(null),[s2,setS2]=useState(null),[s3,setS3]=useState(null),[s4,setS4]=useState(null),[s5,setS5]=useState(null),[s6,setS6]=useState(null),[s7,setS7]=useState(null),[s8,setS8]=useState(null),[s9,setS9]=useState(null),[s10,setS10]=useState(null),[s11,setS11]=useState(null),[s12,setS12]=useState(null),[error,setError]=useState("");
 useEffect(()=>{Promise.all([getSchoolCore(),getSchoolS2Health(),getSchoolS3Health(),getSchoolS4Health(),getSchoolS5Health(),getSchoolS6Health(),getSchoolS7Health(),getSchoolS8Health(),getSchoolS9Health(),getSchoolS10Health(),getSchoolS11Health(),getSchoolS12Health()]).then(([a,b,c,d,e,f,g,h,i,j,k,l])=>{setData(a);setS2(b);setS3(c);setS4(d);setS5(e);setS6(f);setS7(g);setS8(h);setS9(i);setS10(j);setS11(k);setS12(l)}).catch(e=>setError(e.message))},[]);
 const h=data?.health||{};
 const cards=[
