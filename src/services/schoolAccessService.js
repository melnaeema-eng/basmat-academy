...
export async function getCurrentSchoolAccess(){
 const {data,error}=await supabase.rpc("school_current_access");
 if(error)throw error;return data||{roles:[],destinations:{}};
}
export async function getSchoolUserAccessDetails(authUserId){
 const {data,error}=await supabase.rpc("school_user_access_details",{p_auth_user_id:authUserId});
 if(error)throw error;return data||{};
}
export async function getSchoolAccessV4Health(){
 const {data,error}=await supabase.rpc("school_access_v4_health");
 if(error)throw error;return data||{};
}
