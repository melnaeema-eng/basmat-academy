...
const emptyEnrollment={id:null,student_id:"",academic_year_id:"",grade_level_id:"",curriculum_id:"",class_section_id:"",status:"active"};
...
export default function SchoolStudents(){
 ...
 function editEnrollment(student,enrollment){
  setMode("enroll");
  setEnroll({
    id:enrollment.id,
 ...
 async function saveEnrollmentForm(e){
  ...
  try{
    ...
    setEnroll(v=>({...v,id:null,student_id:"",class_section_id:""}));
    await load();
    alert(enroll.id?"تم تحديث التسجيل الأكاديمي":"تم تسجيل الطالب أكاديميًا");
  }catch(e){alert(e.message)}finally{setSaving(false)}
 }
  ...
  <div className="grid gap-6 xl:grid-cols-[460px_1fr]">
   <div>
    {mode==="student"?<form onSubmit={saveStudentForm} className="academy-card space-y-4 p-5">
      ...
      <div className="flex items-center justify-between"><h2 className="font-extrabold text-[#12345b]">{enroll.id?"تعديل التسجيل الأكاديمي":"التسجيل الأكاديمي"}</h2>{enroll.id&&<button type="button" onClick={()=>setEnroll(v=>({...v,id:null,student_id:"",class_section_id:""}))} className="rounded-lg bg-slate-100 p-2 text-slate-500"><FaTimes/></button>}</div>
      <Field label="الطالب"><select required disabled={!!enroll.id} className="academy-input" value={enroll.student_id} onChange={e=>setEnroll({...enroll,student_id:e.target.value})}><option value="">اختر الطالب</option>{students.map(s=><option key={s.id} value={s.id}>{s.student_no} — {s.full_name_ar}</option>)}</select></Field>
      ...
      <button disabled={saving} className="academy-btn-primary w-full"><FaUserGraduate/>{saving?"جاري الحفظ...":enroll.id?"حفظ تعديل التسجيل":"تسجيل الطالب"}</button>
    </form>}
   </div>
