...
export async function saveSchoolAdmission(x){const row={academic_year_id:x.academic_year_id||null,curriculum_id:x.curriculum_id||null,grade_level_id:x.grade_level_id||null,student_name_ar:x.student_name_ar.trim(),student_name_en:x.student_name_en?.trim()||null,gender:x.gender||null,birth_date:x.birth_date||null,nationality:x.nationality?.trim()||null,previous_school:x.previous_school?.trim()||null,parent_name:x.parent_name.trim(),parent_email:x.parent_email?.trim()||null,parent_phone:x.parent_phone.trim(),address:x.address?.trim()||null,notes:x.notes?.trim()||null,updated_at:new Date().toISOString()};if(x.id){const{data,error}=await supabase.from("school_admission_applications").update(row).eq("id",x.id).select().single();if(error)throw error;return data}const{data,error}=await supabase.from("school_admission_applications").insert(row).select().single();if(error)throw error;return data}
