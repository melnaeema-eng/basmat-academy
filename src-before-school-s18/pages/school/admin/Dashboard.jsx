...
import {getSchoolCore,getSchoolS2Health,getSchoolS3Health,getSchoolS4Health,getSchoolS5Health,getSchoolS6Health,getSchoolS7Health,getSchoolS8Health,getSchoolS9Health,getSchoolS10Health,getSchoolS11Health,getSchoolS12Health,getSchoolS13Health,getSchoolS14Health,getSchoolS15Health,getSchoolS16Health} from "../../../services/schoolService";

export default function SchoolDashboard(){
 const[data,setData]=useState(null),[s2,setS2]=useState(null),[s3,setS3]=useState(null),[s4,setS4]=useState(null),[s5,setS5]=useState(null),[s6,setS6]=useState(null),[s7,setS7]=useState(null),[s8,setS8]=useState(null),[s9,setS9]=useState(null),[s10,setS10]=useState(null),[s11,setS11]=useState(null),[s12,setS12]=useState(null),[s13,setS13]=useState(null),[s14,setS14]=useState(null),[s15,setS15]=useState(null),[s16,setS16]=useState(null),[error,setError]=useState("");
 useEffect(()=>{Promise.all([getSchoolCore(),getSchoolS2Health(),getSchoolS3Health(),getSchoolS4Health(),getSchoolS5Health(),getSchoolS6Health(),getSchoolS7Health(),getSchoolS8Health(),getSchoolS9Health(),getSchoolS10Health(),getSchoolS11Health(),getSchoolS12Health(),getSchoolS13Health(),getSchoolS14Health(),getSchoolS15Health(),getSchoolS16Health()]).then(([a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p])=>{setData(a);setS2(b);setS3(c);setS4(d);setS5(e);setS6(f);setS7(g);setS8(h);setS9(i);setS10(j);setS11(k);setS12(l);setS13(m);setS14(n);setS15(o);setS16(p)}).catch(e=>setError(e.message))},[]);
 const h=data?.health||{};
 const cards=[
  ...
  ["عقود",s6?.contracts||0,FaBook],
  ["طلبات إجازة",s6?.leave_requests||0,FaUsers],
  ["كتب رقمية",s7?.books||0,FaBook],
  ["طلبات قبول",s8?.applications||0,FaUsers],
  ["بنك الأسئلة",s9?.question_bank||0,FaBook],
  ["محاولات إلكترونية",s9?.attempts||0,FaGraduationCap],
  ["نتائج سنوية",s10?.annual_results||0,FaGraduationCap],
  ["شهادات",s10?.certificates||0,FaBook],
  ["إشعارات غير مقروءة",s11?.unread_notifications||0,FaUsers],
  ["أسر متعددة الأبناء",s12?.sibling_families||0,FaUsers],
  ["طلاب بحسابات",s13?.students_with_auth||0,FaUsers],
  ["معلمون بحسابات",s14?.teachers_with_auth||0,FaUsers],
  ["Security Status",s15?.security?.status==="OK"?"OK":"CHECK",FaUsers],
  ["طلاب بلا فصل",s16?.unassigned_enrollments||0,FaUserGraduate],
  ["معلمون بلا تكليف",s16?.teachers_without_assignments||0,FaUsers],
 ];
 return <div>
