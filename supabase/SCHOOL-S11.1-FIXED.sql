...
-- school_attendance_sessions  -> attendance_date
-- school_attendance_records   -> enrollment_id + status
...
declare
  ...
  attendance_day date;
begin
  -- Do not send duplicate alerts when an UPDATE keeps the same status.
  ...
  join school_attendance_sessions sess
    on sess.id=new.session_id
  where e.id=new.enrollment_id;

  select p.auth_user_id
  into puser
  from school_parent_students ps
  join school_parents p
  ...
  if puser is not null then
    -- Prevent accidental duplicate same-day alert for same attendance record.
    if not exists(
      select 1
      from school_notifications n
      where n.recipient_user_id=puser
      ...
      perform public.school_notify_user(
        ...
        'attendance',
        case
          when new.status='absent' then 'تنبيه غياب'
          else 'تنبيه تأخير'
        end,
        case
          when new.status='absent'
            then 'تم تسجيل غياب الطالب '||sname||' بتاريخ '||attendance_day
          else 'تم تسجيل تأخير الطالب '||sname||' بتاريخ '||attendance_day
        end,
        'attendance_record',
        new.id,
...
drop trigger if exists trg_school_notify_student_attendance
on public.school_attendance_records;

create trigger trg_school_notify_student_attendance
after insert or update of status
on public.school_attendance_records
for each row
execute function public.school_notify_student_attendance();
