...
import {getSchoolCore,getSchoolS2Health,getSchoolS3Health,getSchoolS4Health} from "../../../services/schoolService";

export default function SchoolDashboard(){
 const[data,setData]=useState(null),[s2,setS2]=useState(null),[s3,setS3]=useState(null),[s4,setS4]=useState(null),[error,setError]=useState("");
 useEffect(()=>{Promise.all([getSchoolCore(),getSchoolS2Health(),getSchoolS3Health(),getSchoolS4Health()]).then(([a,b,c,d])=>{setData(a);setS2(b);setS3(c);setS4(d)}).catch(e=>setError(e.message))},[]);
 const h=data?.health||{};
 const cards=[
